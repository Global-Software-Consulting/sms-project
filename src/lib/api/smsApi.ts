 ...
 *
 * Each upstream provider stores `code` differently — 5sim returns ISO-2
 * ("US"), hero-sms returns the full name ("USA"), sms-man returns a
 * numeric id ("187"). So we try the 2-letter path first, then fall back
 * to a normalized name lookup. Anything we can't match renders as 🌍.
 */
...
export const getCountryFlag = (code: string, name?: string): string => {
  ...
  if (code && code.length === 3 && /^[a-zA-Z]{3}$/.test(code)) {
    const iso2 = NAME_TO_ISO2[code.toLowerCase()];
    if (iso2) return iso2ToFlag(iso2);
  }
  const fromCode = code ? NAME_TO_ISO2[code.toLowerCase()] : undefined;
  if (fromCode) return iso2ToFlag(fromCode);
  if (name) {
