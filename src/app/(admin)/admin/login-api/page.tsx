...
export default function AdminLoginApiPage() {
  ...
  const initialTab: TabType =
    tabFromUrl && validTabs.includes(tabFromUrl) ? tabFromUrl : 'login';
  ...
  const validateLoginForm = (form: LoginSettings): Record<string, string> => {
    ...
    if (form.googleEnabled) {
      ...
      if (!form.googleClientSecret.trim())
        errs.googleClientSecret =
          'Client Secret is required when Google login is enabled';
    }
    if (form.twitterEnabled) {
      if (!form.twitterConsumerKey.trim())
        errs.twitterConsumerKey =
          'Consumer Key is required when Twitter login is enabled';
      if (!form.twitterConsumerSecret.trim())
        errs.twitterConsumerSecret =
          'Consumer Secret is required when Twitter login is enabled';
    }
    if (form.githubEnabled) {
      if (!form.githubClientId.trim())
        errs.githubClientId =
          'Client ID is required when GitHub login is enabled';
      if (!form.githubClientSecret.trim())
        errs.githubClientSecret =
          'Client Secret is required when GitHub login is enabled';
    }
    if (form.telegramEnabled) {
      if (!form.telegramBotToken.trim())
        errs.telegramBotToken =
          'Bot Token is required when Telegram login is enabled';
    }
    return errs;
  };
      ...
      await bulkUpdateSettings({
        settings: [
          // Toggles
          {
            ...
            value: String(loginForm.googleEnabled),
          },
          {
            key: 'login_twitter_enabled',
            value: String(loginForm.twitterEnabled),
          },
          {
            key: 'login_github_enabled',
            value: String(loginForm.githubEnabled),
          },
          {
            key: 'login_telegram_enabled',
            value: String(loginForm.telegramEnabled),
          },

          // Google credentials
          { key: 'login_google_client_id', value: loginForm.googleClientId },
          {
            key: 'login_google_client_secret',
            value: loginForm.googleClientSecret,
          },
          {
            key: 'login_google_callback_url',
            value: loginForm.googleCallbackUrl,
          },

          // Twitter credentials
          {
            key: 'login_twitter_consumer_key',
            value: loginForm.twitterConsumerKey,
          },
          {
            key: 'login_twitter_consumer_secret',
            value: loginForm.twitterConsumerSecret,
          },
          {
            key: 'login_twitter_callback_url',
          ...
          {
            key: 'login_github_client_secret',
            value: loginForm.githubClientSecret,
          },
          {
            key: 'login_github_callback_url',
            value: loginForm.githubCallbackUrl,
          },

          // Telegram credentials
          {
            key: 'login_telegram_bot_token',
            value: loginForm.telegramBotToken,
          },
          {
            key: 'login_telegram_bot_username',
  ...
  const updateLoginField = <K extends keyof LoginSettings>(
    field: K,
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      {/* Tabs */}
      <div className="mb-6 flex items-center gap-4 overflow-x-auto border-b border-[rgba(255,255,255,0.18)] [-ms-overflow-style:none] [scrollbar-width:none] sm:mb-8 sm:gap-6 [&::-webkit-scrollbar]:hidden">
        <button
          onClick={() => handleTabChange('login')}
          className={`size-icon relative !p-0 px-2 pb-3 text-sm font-medium whitespace-nowrap transition-colors sm:pb-4 sm:text-base ${
            activeTab === 'login'
              ? 'text-[#3B82F6]'
          ...
          <AdminGlassCard>
            <div className="mb-8 text-center">
              ...
              <p className="text-sm text-[#94A3B8]">
                Enable social login providers and configure their OAuth
                credentials. Secrets are stored securely and only sent over
                HTTPS.
              </p>
            </div>
            ...
            <div className="mb-8 space-y-6">
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-[#EA4335]/20">
                    <svg
                      className="h-6 w-6"
                      viewBox="0 0 24 24"
                      fill="currentColor"
                  ...
                  <button
                    ...
                    className={`size-icon !min-h-0 !p-0 relative h-6 w-12 shrink-0 rounded-full transition-colors ${loginForm.googleEnabled ? 'bg-[#3B82F6]' : 'bg-[rgba(255,255,255,0.18)]'}`}
                  >
                    <div
                      className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${loginForm.googleEnabled ? 'translate-x-6' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateLoginField('googleClientId', e.target.value)
                      }
                      placeholder="xxxxxxxxxxxx.apps.googleusercontent.com"
                      autoComplete="off"
                      className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.googleClientId ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                    />
                    {loginErrors.googleClientId && (
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type={
                          showLoginSecrets.googleClientSecret
                            ? 'text'
                            : 'password'
                        }
                        value={loginForm.googleClientSecret}
                        onChange={(e) =>
                          updateLoginField('googleClientSecret', e.target.value)
                        }
                        placeholder="Client secret from Google Cloud Console"
                        autoComplete="new-password"
                        className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 pr-12 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.googleClientSecret ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                      />
                      <button
                        type="button"
                        onClick={() => toggleLoginSecret('googleClientSecret')}
                        className="size-icon !min-h-0 !p-0 absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center text-[#64748B] transition-colors hover:text-white"
                      >
                        {showLoginSecrets.googleClientSecret ? (
                    ...
                    {loginErrors.googleClientSecret && (
                      <p className="mt-1 text-xs text-red-400">
                        {loginErrors.googleClientSecret}
                      </p>
                    )}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      value={loginForm.googleCallbackUrl}
                      onChange={(e) =>
                        updateLoginField('googleCallbackUrl', e.target.value)
                      }
                      placeholder={callbackPlaceholder('google')}
                      autoComplete="off"
                      className="w-full rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(255,255,255,0.08)] px-4 py-3 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm"
                    />
                  </div>
                </div>
                ...
                <p className="mt-3 text-xs text-[#64748B]">
                  ...
                  &rarr; APIs & Services &rarr; Credentials. The Authorized
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-[#1DA1F2]/20">
                    <svg
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[#64748B]">
                      Allow users to sign in with Twitter accounts
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      updateLoginField(
                        'twitterEnabled',
                        !loginForm.twitterEnabled,
                      )
                    }
                    className={`size-icon !min-h-0 !p-0 relative h-6 w-12 shrink-0 rounded-full transition-colors ${loginForm.twitterEnabled ? 'bg-[#3B82F6]' : 'bg-[rgba(255,255,255,0.18)]'}`}
                  >
                    <div
                      className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${loginForm.twitterEnabled ? 'translate-x-6' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      value={loginForm.twitterConsumerKey}
                      onChange={(e) =>
                        updateLoginField('twitterConsumerKey', e.target.value)
                      }
                      placeholder="API key from Twitter Developer Portal"
                      autoComplete="off"
                      className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.twitterConsumerKey ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                    />
                    {loginErrors.twitterConsumerKey && (
                      <p className="mt-1 text-xs text-red-400">
                        {loginErrors.twitterConsumerKey}
                      </p>
                    )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type={
                          showLoginSecrets.twitterConsumerSecret
                            ? 'text'
                            : 'password'
                        }
                        value={loginForm.twitterConsumerSecret}
                        ...
                        autoComplete="new-password"
                        className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 pr-12 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.twitterConsumerSecret ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                      />
                      <button
                        type="button"
                        onClick={() =>
                          toggleLoginSecret('twitterConsumerSecret')
                        }
                        className="size-icon !min-h-0 !p-0 absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center text-[#64748B] transition-colors hover:text-white"
                      >
                        {showLoginSecrets.twitterConsumerSecret ? (
                    ...
                    {loginErrors.twitterConsumerSecret && (
                      <p className="mt-1 text-xs text-red-400">
                        {loginErrors.twitterConsumerSecret}
                      </p>
                    )}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      value={loginForm.twitterCallbackUrl}
                      onChange={(e) =>
                        updateLoginField('twitterCallbackUrl', e.target.value)
                      }
                ...
                <p className="mt-3 text-xs text-[#64748B]">
                  ...
                  &rarr; Projects & Apps. Make sure to enable "Request email
                  from users".
                </p>
              </div>
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-[#333]/20">
                    <svg
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[#64748B]">
                      Allow users to sign in with GitHub accounts
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      updateLoginField(
                        'githubEnabled',
                        !loginForm.githubEnabled,
                      )
                    }
                    className={`size-icon !min-h-0 !p-0 relative h-6 w-12 shrink-0 rounded-full transition-colors ${loginForm.githubEnabled ? 'bg-[#3B82F6]' : 'bg-[rgba(255,255,255,0.18)]'}`}
                  >
                    <div
                      className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${loginForm.githubEnabled ? 'translate-x-6' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateLoginField('githubClientId', e.target.value)
                      }
                      placeholder="OAuth App Client ID"
                      autoComplete="off"
                      className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.githubClientId ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                    />
                    {loginErrors.githubClientId && (
                      <p className="mt-1 text-xs text-red-400">
                        {loginErrors.githubClientId}
                      </p>
                    )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type={
                          showLoginSecrets.githubClientSecret
                            ? 'text'
                            : 'password'
                        }
                        value={loginForm.githubClientSecret}
                        onChange={(e) =>
                          updateLoginField('githubClientSecret', e.target.value)
                        }
                        placeholder="OAuth App Client Secret"
                        autoComplete="new-password"
                        className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 pr-12 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.githubClientSecret ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                      />
                      <button
                        type="button"
                    ...
                    {loginErrors.githubClientSecret && (
                      <p className="mt-1 text-xs text-red-400">
                        {loginErrors.githubClientSecret}
                      </p>
                    )}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      value={loginForm.githubCallbackUrl}
                      onChange={(e) =>
                        updateLoginField('githubCallbackUrl', e.target.value)
                      }
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-4">
                  <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-[#0088cc]/20">
                    <svg
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[#64748B]">
                      Allow users to sign in with Telegram accounts
                    </p>
                  </div>
                  <button
                    onClick={() =>
                      updateLoginField(
                        'telegramEnabled',
                        !loginForm.telegramEnabled,
                      )
                    }
                    className={`size-icon !min-h-0 !p-0 relative h-6 w-12 shrink-0 rounded-full transition-colors ${loginForm.telegramEnabled ? 'bg-[#3B82F6]' : 'bg-[rgba(255,255,255,0.18)]'}`}
                  >
                    <div
                      className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${loginForm.telegramEnabled ? 'translate-x-6' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type={
                          showLoginSecrets.telegramBotToken
                            ? 'text'
                            : 'password'
                        }
                        value={loginForm.telegramBotToken}
                        onChange={(e) =>
                          updateLoginField('telegramBotToken', e.target.value)
                        }
                        placeholder="123456:ABC-DEF..."
                        autoComplete="new-password"
                        className={`w-full rounded-xl border bg-[rgba(255,255,255,0.08)] px-4 py-3 pr-12 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm ${loginErrors.telegramBotToken ? 'border-red-500' : 'border-[rgba(255,255,255,0.18)]'}`}
                      />
                      <button
                        type="button"
                ...
                <p className="mt-3 text-xs text-[#64748B]">
                  ...
                  on Telegram to get the Bot Token. The Bot Username is the
                  @name (without the @).
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end sm:gap-4">
              <button
                ...
                className="flex w-full items-center justify-center rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(255,255,255,0.08)] px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-[rgba(255,255,255,0.12)] sm:w-auto sm:justify-start"
              >
                Refresh
              </button>
              <button
                ...
                disabled={isLoading}
                className="flex w-full items-center justify-center rounded-xl bg-[#3B82F6] px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-[#2563EB] disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:justify-start"
              >
        ...
        <div>
          <AdminGlassCard>
            <div className="mb-8 text-center">
              ...
              <p className="text-sm text-[#94A3B8]">
                Manage API keys and configuration for various services. All
            ...
            <div className="mb-8 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(255,255,255,0.08)] px-4 py-3 pr-12 text-base text-white focus:ring-2 focus:ring-[#3B82F6] focus:outline-none lg:text-sm"
                      />
                      <button
                ...
                <div className="mt-4 flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setApiConfig({
                        ...apiConfig,
                        useSslTls: e.target.checked,
                      })
                    }
                    className="!min-h-0 h-4 w-4 rounded border-[rgba(255,255,255,0.18)] bg-[rgba(255,255,255,0.08)] text-[#3B82F6] focus:ring-[#3B82F6]"
                  />
                  <label htmlFor="useSslTls" className="text-sm text-white">
                    Use SSL/TLS
                  </label>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      ...
                      <span className="text-[#3B82F6]">
                        (Swedish, Norwegian, Danish, Finnish, French, German,
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-3">
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-[#EA4335]/20">
              ...
              <div className="rounded-xl border border-[rgba(255,255,255,0.18)] bg-[rgba(0,0,0,0.3)] p-4 sm:p-6">
                <div className="mb-6 flex items-start gap-3">
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-[#EF4444]/20">
